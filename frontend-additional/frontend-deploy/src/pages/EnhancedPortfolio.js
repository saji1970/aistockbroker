...
const EnhancedPortfolio = () => {
  ...
  const buyStockMutation = useMutation(
    async ({ symbol, shares }) => {
      return portfolioAPI.buyStock(symbol, shares, currentMarket);
    },
    {
      onSuccess: (data) => {
        queryClient.invalidateQueries(['portfolio', currentMarket]);
  ...
  const sellStockMutation = useMutation(
    async ({ symbol, shares }) => {
      return portfolioAPI.sellStock(symbol, shares, currentMarket);
    },
    {
      onSuccess: (data) => {
        queryClient.invalidateQueries(['portfolio', currentMarket]);
                        ...
                        <tbody className="divide-y divide-gray-200">
                          {portfolioData.positions.map((position, index) => (
                            <tr key={index} className="hover:bg-gray-50">
                              ...
                              <td className={`py-3 px-4 text-right font-medium ${position.unrealized_pnl >= 0 ? 'text-green-600' : 'text-red-600'}`}>
            ...
            <div className="p-6">
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                ...
                <div>
                  ...
                  <p className={`text-lg font-semibold ${selectedPosition.unrealized_pnl >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                ...
                <div>
                  ...
                  <p className="text-lg font-semibold">
                    {new Date(selectedPosition.last_updated).toLocaleString()}
                  </p>
                </div>
              </div>
